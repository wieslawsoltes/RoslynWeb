...
public static partial class CompilerBridge
{
    ...
    {
        Initialize();
        return Serialize(new { bridgeVersion = "0.7.0", roslynVersion = typeof(CSharpCompilation).Assembly.GetName().Version?.ToString(), runtimeVersion = Environment.Version.ToString(), referenceCount = References.Count, execution = "dotnet-wasm-interpreter" });
    }
