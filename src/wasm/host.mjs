...
import {AssemblyCompilerHost, BoundedCache, canonical, copy} from '../compiler-cache.mjs';
...
export class NativeWasmHost extends AssemblyCompilerHost {
  ...
  async emit(input, options = {}) {
    const prepared = await this.prepare(input, options, key => this.emissions.get(key));
    try {
      this.assertPrepared(prepared);
      const {key, start} = prepared;
      const linked = now();
      let artifact = prepared.reused || this.emissions.get(key), emitHit = !!artifact;
      if (!artifact) { artifact = compileWasm(prepared.model, prepared.options); this.emissions.set(key, artifact); }
      const end = now();
      return {...artifact, format: 'wasm', success: true, cache: {emitHit}, timings: {
        ...artifact.timings, ...(emitHit ? {analysisMs: 0, emissionMs: 0} : {}),
        ...prepared.timings, emitMs: end - linked + (prepared.reuseMs || 0), totalMs: end - start
      }};
  ...
  async compile(request, options = {}) {
    ...
    const start = now();
    const assembly = await this.callManaged('Compile', [JSON.stringify({...request, includeInspection})]);
    this.assertActive();
  ...
  clearCompilations(marker) {
    if (marker === undefined) { this.emissions.clear(); return; }
    let changed = false;
    for (const key of this.emissions.items.keys()) if (key.includes(marker)) changed = this.emissions.delete(key) || changed;
    return changed;
  }
