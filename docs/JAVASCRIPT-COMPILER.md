...
| `System.Nullable<T>` | Default/constructor, HasValue/Value/GetValueOrDefault, supported compiler-generated lifted operations, null/underlying-value boxing and unboxing, Equals and Compare. | General arbitrary-struct hashing and complete generic framework behavior are not supplied. Hash support is limited to documented integral/Decimal values. |
| `System.ValueTuple` | Arity 0–8 with nested Rest, Create/default values, fields, copies, byrefs, arrays, boxing, Equals, CompareTo and ToString. | ITuple indexing, structural comparer overloads and tuple hashing are not supplied. |
