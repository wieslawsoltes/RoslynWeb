...
try {
 ...
 await test('Records, generics and LINQ',()=>run(examples[2].source,'Temperature: 23.15\nPressure: 101.10'));
 await test('Async Task Main and exception/finally',()=>run(examples[3].source,'Starting asynchronous work…\nCaught: FormatException\nFinally block executed.\nAsync work complete.'));
 await test('System.Text.Json and reflection',()=>run(examples[4].source,'{"Name":"Ada","Age":36}\nName = Ada\nAge = 36'));
 await test('Portable DLL binary metadata inspection',async()=>{const c=await compile(examples[1].source);const m=await compiler.inspect(c);const methods=m.types.flatMap(t=>t.methods);assert.ok(methods.some(m=>m.name==='Fibonacci'));assert.ok(methods.some(m=>(m.body||m.instructions||[]).some(i=>(i.opcode||i.opCode)==='add')));});
 await test('Run generated JavaScript from actual C# MSIL',()=>run(examples[1].source,'Array total:\n30\nFibonacci(12):\n144','javascript'));
 ...
 await test('Load official NuGet archive and execute Newtonsoft.Json',async()=>{const p=await compiler.importPackage(new Uint8Array(await readFile(new URL('../tests/fixtures/newtonsoft.json.13.0.3.nupkg',import.meta.url))));assert.equal(p.id.toLowerCase(),'newtonsoft.json');await run(examples[6].source,'Roslyn Browser\n{"Ready":true,"Answer":42}');});
