...
function dirty() { sourceVersion++; $('dirty').hidden = false; artifact = null; jsSource = ''; controls(); }
...
function tab(name) {
  ...
  for (const n of ['source', 'il', 'js', 'generated', 'host']) $('panel-' + n).hidden = n !== name;
}
function diagnostics(items = []) {
  $('diagnostics').replaceChildren();
  for (const diagnostic of items) {
    const button = document.createElement('button'); button.className = 'diagnostic';
    button.textContent = `${diagnostic.severity} ${diagnostic.id} · ${diagnostic.path || 'Program.cs'}:${diagnostic.startLine || 1}:${diagnostic.startColumn || 1} — ${diagnostic.message}`;
    button.onclick = () => {
...
async function compile() {
  ...
  const extensionOptions=await prepareExample(compiler,example);
  if(example?.kind==='project') {
    const build=await compiler.buildProject({projectPath:'Demo.csproj',files:projectFiles(source.value),restore:false});
    artifact=build.compileResult || {success:false,diagnostics:build.diagnostics};
  } else if(example?.kind==='dynamic') {
    const fn=await compiler.compileFunction({name:'Multiply',returnType:'long',parameters:[{name:'value',type:'long'},{name:'factor',type:'long'}],body:source.value});
    artifact=fn.assembly || fn;
  } else artifact = await compiler.compile(source.value, { optimization: 'release', nullable: 'enable', emitPdb: true, includeInspection: true, ...extensionOptions, outputKind:['objects','desktop'].includes(example?.kind)?'library':'console' });
  $('panel-generated').textContent=(artifact.generatedSources||[]).map(g=>`// ${g.hintName||g.path||g.name}\n${g.text||g.source||''}`).join('\n\n') || 'No generated sources for this compilation.';
  compiledVersion = version;
  diagnostics(artifact.diagnostics);
  $('timing').textContent = `${(performance.now() - started).toFixed(0)} ms`;
  if (!artifact.success) { if (artifact.error) log(artifact.error.message); status('Compilation failed'); log('Compilation failed. Select a diagnostic to jump to the source.'); return false; }
  $('dirty').hidden = sourceVersion === version;
  log(`Emitted ${artifact.assemblyName}.dll · ${artifact.pe.length.toLocaleString()} bytes`);
  const model = artifact.inspection || await compiler.inspect(artifact);
  $('panel-il').textContent = ilText(model);
  try {
    const result = await compiler.emitJavaScript(artifact, { runtimeImport: '../src/il/runtime.mjs' });
...
async function operation(action) {
  if (busy) return;
  busy = true; controls();
  try { await action(); } catch (error) { status('Error'); log(error.message); if (error.details) log(JSON.stringify(error.details, null, 2)); }
...
$('run').onclick = () => operation(async () => {
  ...
  if (!artifact?.success || compiledVersion !== sourceVersion) if (!await compile()) return;
  status('Running'); log(`Running on ${$('backend').selectedOptions[0].text}…`);
  const started = performance.now();
  const example=examples[Number($('example').value)];
  let result;
  if(example?.kind==='objects') {
    const handle=await compiler.createObject(artifact.assemblyId,'Counter',[10]);
    await compiler.setProperty(handle,'Value',20);
    result=await compiler.invokeObject(handle,'Add',[22]);
    ...
    result=await compiler.invoke(artifact.assemblyId,'RoslynWeb.Dynamic.Function','Multiply',[9007199254740993n,2n]);
    result={...result,backend:'wasm',stdout:`Generated function result: ${result.result}\n`};
  } else if(example?.kind==='desktop') {
    result=await compiler.invoke(artifact.assemblyId,'DesktopDemo','Model',[]);
    if(result.success) { await showDesktop(result.result,artifact.assemblyId); tab('host'); }
    result={...result,backend:'wasm',stdout:'Browser controls connected to the managed DesktopDemo class.\n'};
  } else result = await compiler.run(artifact, { backend: $('backend').value, args, timeoutMs: 30000 });
...
$('file').onchange = () => operation(async () => {
  for (const file of $('file').files) {
    ...
    if (/\.nupkg$/i.test(file.name)) { const p = await compiler.importPackage(bytes); recordPackage(`${p.id} ${p.version}`); for (const warning of p.warnings || []) log(String(warning)); if (p.dependencies?.length) log('This local package declares dependencies. Restore those packages separately before running.'); }
    else { await compiler.addDll(file.name, bytes); recordPackage(file.name); }
  }
  artifact = null; jsSource = ''; status('References loaded'); $('file').value = '';
});
$('restore').onclick = () => operation(async () => {
  ...
  const result = await compiler.restore([{ id, version: version.startsWith('[') || version.startsWith('(') ? version : `[${version}]` }]);
  ...
  artifact = null; jsSource = ''; status('Package restored'); log('Compile and run to use the restored package.');
});
...
function choose() { const e = examples[Number($('example').value)]; source.value = e.source; $('example-description').textContent = e.description; dirty(); lines(); tab('source'); if (e.name.includes('NuGet:')) { $('package-id').value = 'Newtonsoft.Json'; $('package-version').value = '13.0.3'; } }
...
async function showDesktop(model,assemblyId) {
  // The controls are a browser host; managed methods own the application behavior.
  const panel=$('panel-host');panel.replaceChildren();
  const {BrowserDesktopHost}=await import('../src/hosting/index.js');
  ...
  const host=new BrowserDesktopHost({root:panel,onEvent:async event=>{
    if(event.id!=='increment'||event.type!=='click')return;
    const response=await compiler.invoke(assemblyId,'DesktopDemo','Increment',[value]);
