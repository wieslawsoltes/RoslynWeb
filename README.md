...
Version 0.7 adds 73 exact numeric intrinsic overloads, including floating-point classification, primitive bit operations, Sign, Clamp and adjacent representable values. The JavaScript optimizer also specializes proven Int64/UInt64/Single/Double leaf methods. Large integer-to-Single conversions preserve native .NET rounding, and reference modes remain available for comparison. Warm PE emissions reuse validated preparation state, and simultaneous inspections of the same PE share a bridge request. See the [measured performance and limits](docs/COMPILATION-PERFORMANCE.md#typed-numeric-compiler-measurements).
...
| netDxf and WebGPU | Complete pinned source compiled by Roslyn WASM, managed DXF reader/writer sessions, text/binary import/export and a reusable WebGPU CAD viewport |
